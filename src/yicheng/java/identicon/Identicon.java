...
public class Identicon {
	...
	private Color getHashColor() {
		return Color.decode(Integer.toHexString(this.name.hashCode()));
	}
			...
			for (int i = 50; i < this.width - 50; i = i + 1) {
				...
				if (y <= 2) {
					if (hashValues[x + y * 5] >= 0) {
						this.bufferedImage.setRGB(j, i,
								this.BLOCK_COLOR.getRGB());
					}
				}
				else {
	...
	public static void main(String[] args) {
		...
		Identicon identicon = new Identicon("yichengzhang", new Color(225, 97,
				56));
		try {
			identicon.saveToPath("C:\\Users\\ZhangY\\Desktop");
		}
		catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
